        ...
        Pass
        {
            ...
            float4 Frag(Varyings i) : SV_Target
            {
                ...
                for(int i = 0; i < int(sampleCount); i ++){
                    ...
                    float3 samplePos = positionVS + offset * radius *  (1 + i) * rcpSampleCount;
                    float2 sampleUV = ReProjectToUV(samplePos);
                    ...
                    float a = max(0,dot(hitOffset,normalVS) - 0.001);
                    float b = dot(hitOffset,hitOffset) + 0.001;
                    ao += a * rcp(b);
                }
